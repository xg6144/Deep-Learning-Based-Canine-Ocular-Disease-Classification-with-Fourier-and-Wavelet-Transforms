...
def save_training_plots(train_losses, train_accs, val_losses, val_accs):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('loss_plot_fourier_combine.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('accuracy_plot_fourier_combine.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def test_model(model, test_loader, device, image_datasets):
    ...
    plt.title('Confusion Matrix (fourier_combine)')
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.savefig('confusion_matrix_fourier_combine.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    sns.heatmap(cm_norm, annot=True, fmt='.2f', cmap='Blues', 
                xticklabels=image_datasets['test'].classes, yticklabels=image_datasets['test'].classes)
    plt.title('Normalized Confusion Matrix (fourier_combine)')
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.savefig('confusion_matrix_normalized_fourier_combine.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title('ROC Curves for Each Class (fourier_combine)')
    plt.legend(loc='lower right')
    plt.grid(True)
    plt.savefig('roc_curves_fourier_combine.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.ylabel('Density')
    plt.title('Confidence Score Distribution (fourier_combine)')
    plt.legend()
    plt.grid(True)
    plt.savefig('confidence_distribution_fourier_combine.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with open('final_results_multi.json', 'w') as f:
...
def main():
    ...
    data_dir_fourier = '/home/dongbeen/ML/dog_eye_classification/data/split_data_fourier/'
    
    image_datasets = {
        'train': DogEyesDataset(
            os.path.join(data_dir_original, 'train'),
            os.path.join(data_dir_fourier, 'train'),
        ...
        'val': DogEyesDataset(
            ...
            os.path.join(data_dir_fourier, 'validation'),
        ...
        'test': DogEyesDataset(
            ...
            os.path.join(data_dir_fourier, 'test'),
